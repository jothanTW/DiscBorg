            ...
            try
            {
                ...
                discord.MessageCreated += async (client, e) =>
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        //Console.Out.WriteLine(ex.GetType().ToString() + ": " + ex.Message + ": Line " + new StackTrace(ex, true).GetFrame(0).GetFileLineNumber());
                    }
                };
                ...
                discord.GuildAvailable += async (client, e) =>
                {
                    ...
                    if (numGuilds == 0)
                    {
                        ...
                        DiscordActivity activity = new DiscordActivity(RandomGameName.getGameName(), ActivityType.Playing);
                        await discord.UpdateStatusAsync(activity);
                    }
                };

                /*
                discord.DmChannelCreated += async (client, e) =>
                {
                ...
                */
                ...
                try
                {
                    while (!exitSource.IsCancellationRequested)
                    {
                        ...
                        await discord.UpdateStatusAsync(activity);
                        await Task.Delay(1800000, exitSource.Token);
                        activity.Name = RandomGameName.getGameName();
                        await discord.UpdateStatusAsync(activity);
                    }
                }
        ...
        static async Task loadLastMessages(DiscordChannel chan)
        {
            if (chan.Type == ChannelType.Group || chan.Type == ChannelType.Private || chan.Type == ChannelType.Text)
            {
                ...
                if (cInfo.lastMessage.Length > 0)
                {
                    ...
                    do
                    {
                        IReadOnlyList<DiscordMessage> messages = await chan.GetMessagesAfterAsync(Convert.ToUInt64(cInfo.lastMessage), maxMessageLook);
                        mRec = messages.Count;
                        if (mRec == 0)
                            break;
                        tmRec += mRec;
